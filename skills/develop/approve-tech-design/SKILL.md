...
1. 运行 `crctl approve {cr_id} --stage tech-design --grant --approver {cr.md owners.development.id}`（平台非 TTY，默认 grant 落点；**仅限 Ed25519 签名授权或人类交互式终端二选一，两者都不可绕过审批本身**）——crctl 自动完成：
...
2. 本地独立 CLI（无 grant）：`crctl approve {cr_id} --stage tech-design --approver {cr.md owners.development.id}`，仅限交互式终端；非 TTY 拒绝（APPROVAL_REQUIRES_HUMAN）。
