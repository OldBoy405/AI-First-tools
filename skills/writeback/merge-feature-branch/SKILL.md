...
| `MERGE_SOURCE_MISSING` / `RELEASE_REMOTE_NOT_PUSHED`（CR-2026-044） | publication lag：本地证据有效但远端 requirement ref 缺失或滞后。状态保持 `code-approved`，不回退；按 `error.recovery`（结构化 argv，`shell:false`）**在本 run 内就地执行一次**，随后在**同一 run 内重跑 merge**；不得转成新委派/新 task，也不得为 `push-progress` / checkpoint 单独开委派（CR-2026-066 FR-6） |
