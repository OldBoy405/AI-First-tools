...
| `MERGE_SOURCE_MISSING` / `RELEASE_REMOTE_NOT_PUSHED`（CR-2026-044） | publication lag：本地证据有效但远端 requirement ref 缺失或滞后。状态保持 `code-approved`，不回退；按 `error.recovery`（结构化 argv）先 checkpoint 再重跑 merge |
