...
| 事实 | 值 |
|---|---|
| 参与仓解析 | 仅 `dir-graph.yaml#repositories` 声明的 active 仓有 worktree；**tools 仓（`../tools`，phase0-tools）自 CR-2026-027 起已声明为参与仓（trunk=**custom/main**，role=code）**，与其他 active 仓统一遍历：注册、同步、合并、清理均只读 repositories，无任何 prose 硬编码特例（FR-5） |
