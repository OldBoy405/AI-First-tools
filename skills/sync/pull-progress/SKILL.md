...
2. 解析所有 `active != false` 的 repo，经 `crctl workspace inspect {cr_id} --workspace <ws>（resources[].worktreePath 为权威路径）` 取权威 worktreePath（FR-29②，CR-2026-022——不再手拼 bucket/worktreePath；bucket 规则 `role=knowledge-base → knowledge-base，其余 → repo.id` 由 crctl 唯一派生）：
3. 确认每个 active repo 都存在本地 worktree 分支 `requirement/{cr_id}`（通过 `crctl git worktree list --cwd <worktree>` 或等价受控查询检查）。
