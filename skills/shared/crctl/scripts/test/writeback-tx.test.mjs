...
import {
  ...
  resolveRepositories, resolveWritebackCandidate, writebackInputDigest,
...
test('CR-2026-038 TASK-02：内部 baseline 路径同一 commit 发布状态并在 origin-confirmed 后投影', async () => {
...
test('CR-2026-038 TASK-02：journal-created 恢复冻结 transitionAt，业务参数漂移硬阻断', async () => {
  const { base, kb, cr, txws } = makeMergedFixture();
  try {
    const ctx = resolveRepositories(kb);
    const observed = { status: [], audit: [] };
    const input = {
      cr, stage: 'baseline', specId: 'test-spec', targetVersion: '0.2', workspace: kb,
      ...atomicCallbacks(txws, cr, observed),
    };
    ...
    const journalBefore = JSON.parse(fs.readFileSync(path.join(txDir, txId, 'journal.json'), 'utf8'));
    ...
    assert.ok(crMd.includes(`updated-at: "${journalBefore.createdAt}"`));
  } finally {
    delete process.env.CRCTL_FAULT_POINT;
...
test('CR-2026-038 TASK-02：投影失败不反转 Git，重放只补缺项', async () => {
  const { base, kb, cr, txws } = makeMergedFixture();
  try {
    const ctx = resolveRepositories(kb);
    const observed = { status: [], audit: [] };
    ...
    assert.equal(observed.status.length, 1, '只补 status');
    assert.equal(observed.audit.length, 1, '已成功 audit 不重复');
    assert.equal(git(path.join(base, 'origin-kb.git'), ['rev-parse', 'master']), head, '不新增 commit');
  } finally { fs.rmSync(base, { recursive: true, force: true }); }
});
