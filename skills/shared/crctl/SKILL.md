...
| `writeback-apply` | candidate-only writeback 应用（CR-2026-031 TASK-08）：`--stage baseline|tasks|traceability --candidate <manifest.json> --spec-id <id>`——manifest v1 全矩阵校验（schema/allowlist/path 安全/symlink parent/blob 哈希/before 磁盘字节锚点/inputDigest/stage-generator 绑定，恶意 case 零写入）→ txws 精确 stage + staged set 断言 → commit+trailer → lease push + classify；未发布遇 origin 前进 WRITEBACK_REMOTE_STALE（txws 重置后重跑 generator），已发布丢失 WRITEBACK_REMOTE_HISTORY_REWRITTEN | writeback 阶段 |
| `archive` | 单一幂等归档（CR-2026-031 TASK-09）：`[--spec-id <id>]`——四账本（cr.md/_backlog/_history/_index）同批 recoverable write-set + archive commit + lease push；origin confirmed 后 cleanup（txws/CR worktree/本地 ref，clean 才删），失败返回 phase=cleanup-pending（status 恒 archived）重跑只续清理；rejected/withdrawn 远端未合并 ref 保留为 preservedRefs | cr-archive |
