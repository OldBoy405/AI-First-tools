...
| Add a deep primitive / transaction | [Transactions](/openwiki/operations/crctl-transactions.md) | `lib/durable-tx.mjs`, `lib/workspace-transactions.mjs` | `registerCr`, `checkpointCr`, `mergeCr`, `applyWriteback`, `archiveCr`, `buildRecovery` | `test/{register,checkpoint,merge,writeback,archive}-tx.test.mjs` | `node skills/shared/crctl/scripts/test/suite-gate.mjs --run` |
...
- **Late-stage rollback transitions** (CR-2026-061 / AIFI-17): three new recovery edges in the [state machine](/openwiki/architecture/overview.md) — `requirement-approved → drafting`, `developing → tech-design-reviewed`, `merging → code-approved` — plus the optional promotion-bind step in `requirement-register`.
- **Fail-closed review/reset governance** (CR-2026-063): `gate --mode pre-review` hard-fails with `contractDrift` + structured recovery on non-`requirement-reviewing` targets; `review-loop reset` moved onto an atomic single-file ledger transaction with commit isolation and rollback.
- **Structured recovery argv contract** (CR-2026-064): every deep primitive returns one `recovery` object (`executable` + `args[]` + `cwd` + `requiresTTY` + `promptFor[]`) and errors carry it under `error.recovery`; consumers spawn it with `shell: false`. See [crctl transactions & deep primitives](/openwiki/operations/crctl-transactions.md).
- **Test-suite gate** (CR-2026-065): CI now runs the full crctl suite through `suite-gate.mjs`, comparing per-file TAP counts against `gate-registry.json#manifest.cases` to prevent silent test loss, and outbox dedup logic is centralized in `lib/outbox-contract.mjs`.
