...
- 触发修订的变更：新增/删除 skills 顶层分组、Pipeline 结构性变化、crctl 新增写入子命令、状态机口径变化、否决一个重大方案。
  ...
  - 已登记：CR-2026-031 TASK-08 candidate-only writeback 与 writeback-apply（不改状态机/gates/§5 判据）——三个 writeback generator（prd-sdd/tasks/traceability）收敛为 candidate-only：只读 workspace、只输出 candidate 目录（文件 + blobs/ + manifest.json v1：files 唯一/POSIX 字典序/仅 create-replace，inputDigest = canonical 自校验 sha256，两侧公式独立内联由测试交叉验证防漂移）；traceability 改读 change-requests/{cr}/merge-commits.yml（TASK-07 事实源，trunk 取自 dir-graph，删 _backlog.yml 提取）；新增 active 命令 `writeback-apply <cr> --stage baseline|tasks|traceability --candidate <manifest> --spec-id <id>`（事务唯一实现在 applyWriteback：authority 解析经 merge journal operationalWorkspace（resolveOperationalWorkspace 修正——CR worktree 是只读被评审源，post-finalize 事实来自 journal）、manifest 全矩阵校验（schema/allowlist/path 安全/symlink parent/blob 存在与哈希/before=磁盘字节锚点/inputDigest/stage-generator 绑定）、txws 精确 stage + staged set 断言、commit+trailer、lease push + classify；before/after CAS 锚点统一为磁盘字节 sha256（Windows autocrlf CRLF 不影响锚点一致性）；STALE 时 txws 重置到新 origin + 零副作用 journal 清理，重跑 generator 后重试；before 校验幂等重入跳过）；writeback-lib 增 readHashRaw/writeCandidate；FAULT_POINTS 新增 writeback-after-apply/after-commit/after-push 与 archive-after-commit/after-push/during-cleanup（TASK-09 预留）；测试：writeback.test.mjs 10（candidate-only 零写/排序/inputDigest 交叉验证/幂等）+ writeback-tx.test.mjs 6（happy path 精确 staged+trailer+幂等、15 类恶意 manifest 矩阵全 hard fail 零写入、STALE 重建闭环、history rewrite 硬阻断、fault 续跑不重复 commit、traceability 全链路），三 bare remote fixture 提取到 test/merge-fixture.mjs 共享。
  - 已登记：CR-2026-031 TASK-09 archive 与 cleanup-pending（不改状态机/gates/§5 判据）——新增 active 命令 `archive <cr> [--spec-id <id>]`（事务唯一实现在 archiveCr：authority=writing-back→txws / rejected-withdrawn→主 checkout（状态权威在主 checkout，TASK-06 模型）；四账本（cr.md + _backlog.yml 移出 + _history.yml 追加含 notify-log + _index.yml 终态）同批 recoverable write-set → archive commit + trailer → lease push + classify（rebuild 从新 base 重建 write-set，编辑是纯函数可重算）；origin confirmed 后 journal 转 cleanup-pending，逐单元清理 txws/CR worktree/本地 requirement 分支（clean 才删，dirty/unknown 零删除保留在 remaining），archived 删远端 ref、rejected/withdrawn 远端 ref 保留为 preservedRefs；cleanup 失败（含 fault）不抛错返回 phase=cleanup-pending（status 恒 archived），重跑每轮全新尝试只续清理、complete 幂等重放短路（cleanup 后 CR worktree 已删，authority 解析跳过）；archiveLedgerEdits/archiveRecipients 独立实现（TxError 风格，旧 cmdArchiveMove 保留至 TASK-10 删除）；FAULT_POINTS 启用 archive-after-commit/after-push/during-cleanup。测试 test/archive-tx.test.mjs 4（happy path 四账本同批+cleanup 全清+幂等、cleanup fault→cleanup-pending 重跑不重复 commit、dirty worktree 零删除保留、rejected preservedRefs）。
  - 已登记：CR-2026-031 TASK-10 Skill/Pipeline/Agent 契约收敛（不改状态机/gates/§5 判据）——删除公开旧命令 `cr-init`/`worktree-path`/`merge-metadata`/`archive-move`/`--caller`/`git commit --template`（含 applyCommitTemplate/tryReadCrOwnerProjection/editArchiveMove 等专属 helper 与对应测试约 600 行）；register/merge/writeback-apply/archive 成为注册、合并、回写、归档的唯一入口；收敛 Skill 文本（merge-feature-branch 一次 crctl merge、requirement-register 一次 crctl register、writeback×3 为 candidate-only generator + writeback-apply、cr-archive 一次 crctl archive，均只保留前置确认/一次深原语调用/结果分类，不写 Git 命令序列、不手写账本）；sync 组 worktree-path 改 workspace inspect；pipeline JSON（requirement-authoring/feature-writeback）节点 prompt 全部改深原语调用、全部 JSON 可解析；crctl SKILL.md 命令表删除旧命令行；lint-prompts/skill matrix/agent contract 全通过；CR-2026-029 静态断言改收敛形态。
  - 已登记：CR-2026-031 TASK-11 upgrade-check（临时只读预检，SDD §4.6，不改状态机/gates/§5 判据）——新增命令 `upgrade-check`（零写入：fetch 后只读 origin trunk 的 backlog/cr.md status + 本机 merge journal 发布事实，不创建 workspace、不修改审批、不合成 snapshot）；分类：developing 及之前/终态 = safe，旧 code-approved 零 publish = requiresReapproval（切协议后重核 release-subjects），merging/writing-back/partial-publish/authority-unknown = blocksUpgrade（保守），未知 status 一律阻断；canActivate=false 时 exit 1；temporary=true 标记 + CUSTOM-TODO-009 删除条件。测试 test/upgrade-check.test.mjs 2（四类 legacy fixture 分类准确 + 零写入断言、canActivate/exit 码）。
  - 已登记：CR-2026-031 TASK-12 文档/contracts/CI/协议切换（不改状态机/gates/§5 判据）——新增 `.github/workflows/crctl-ci.yml` 双平台（ubuntu+windows）CI：lint-prompts / skill matrix / agent contract / pipeline JSON parse / crctl 全量测试 / writeback 单测；主工作区 AGENTS.md 纪律 2 状态机口径 27/49 → 28/50；multica governance transitions_gen.go 经 gen 重新生成（25/47 → 28/50，source tools@a1c1ccb，CUSTOM.md 编号 4 登记）；README.md 注册段改 crctl register；upgrade-check 主工作区实跑 canActivate=true（11 safe 无 blocker）；旧命令残留扫描清零（仅历史注释）；全部 12 TASK done。合 §4 依赖方向与 §5 全部不变量。
