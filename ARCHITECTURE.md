...
8. **Skill 通用、约束归仓**：`skills/` 下的 SKILL.md 只描述流程、契约与通用工程原则，**不得**内置任何单个使用方仓库的产品专属约束（如表结构、迁移框架规则、锁原语、特定 DDL 规范）；这些约束以目标仓自己的 `ARCHITECTURE.md`/迁移约束为唯一判据，由 `write-tech-design`/`review-tech-design` 读取目标仓后生效（核查：SKILL.md 中出现具体产品专属名词/命令且非「以目标仓为准」式的指引，即属越界，应上移到目标仓文档）。
