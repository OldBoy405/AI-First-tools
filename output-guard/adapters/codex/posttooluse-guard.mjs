#!/usr/bin/env node
// Codex PostToolUse hook → OutputGuard Core 映射（CR-2026-069 TASK-04，SDD §3.3）。
//
...
export async function handle(payload) {
  ...
  try {
    if (payload.hook_event_name !== 'PostToolUse') return { ok: true, out: null };
    const body = bodyOf(payload.tool_response);
    ...
    if (result.action === 'passthrough') return { ok: true, out: null };
    if (result.action === 'unavailable') {
      if (result.reason !== 'structure-not-preserved') reportUnavailable('BUNDLE_INVALID');
      return { ok: true, out: { decision: 'block', reason: body.text + String.fromCharCode(10) + result.trailer, hookSpecificOutput: { hookEventName: 'PostToolUse', additionalContext: 'OutputGuard capped this result (complete=false)' } } };
    }
    return { ok: true, out: { decision: 'block', reason: result.body + String.fromCharCode(10) + result.trailer, hookSpecificOutput: { hookEventName: 'PostToolUse', additionalContext: 'OutputGuard capped this result (complete=false)' } } };
  } catch {
