#!/usr/bin/env node
// CodeBuddy Code PostToolUse hook → OutputGuard Core 映射（CR-2026-069 TASK-04，SDD §3.3）。
//
...
export async function handle(payload) {
  ...
  try {
    ...
    if (result.action === 'unavailable') {
      ...
      return { ok: true, out: { hookSpecificOutput: { hookEventName: 'PostToolUse', updatedToolOutput: replaceBody(payload.tool_response, body.text + String.fromCharCode(10) + result.trailer) } } };
    }
    return { ok: true, out: { hookSpecificOutput: { hookEventName: 'PostToolUse', updatedToolOutput: replaceBody(payload.tool_response, result.body + String.fromCharCode(10) + result.trailer) } } };
  } catch {
